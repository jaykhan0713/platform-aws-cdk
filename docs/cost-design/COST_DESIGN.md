...
To mitigate costs for the portfolio, a single account is used with only "prod" environment in mind. This project is able to be tweaked for single account multi-env (test, prod, etc) as well as
...
0.5 vCPU that's shared between the 3 containers. I have CPU scaling in place for demoing load tests but I can still push my services to handle 200+ RPS with low P99  
~ low double digits ms latency at the edge,  (includes calling other microservices/resources) and single digit ms latency at downstream services. If it does exceed what these small tasks are capable of, it will auto scaling during these load tests.
...
ECS Container insights is a CDK context toggleable setting. The cost is high so I generally only turn on during load-tests when I am needing to optimize CPU and RSS memory for the whole task and see
how each individual container is using these resources. Helps to allocate when tuning app container's resource allocation against adot-collector and envoy sidecar to squeeze the performance out of low-end tasks.

For sleep and wake scripts refer to [OPS documentation](../../ops/README.md)
